...
from uploads.core.forms import DocumentForm, PostForm
...
def simple_upload(request):
    if request.method == 'POST' and 'myfile' in request.POST and request.FILES['myfile']:
...
#Form for a single sample
def post_new(request):
    if request.method == "POST" and 'single_sample' in request.POST:
        form = PostForm(request.POST)
        if form.is_valid():
            myfile = request.POST.get('myfile', False)
            SiO2 = request.POST.get('SiO2')
            TiO2 = request.POST.get('TiO2', None)
            Al2O3 = request.POST.get('Al2O3', None)
            Fe2O3 = request.POST.get('Fe2O3', None)
            FeO = request.POST.get('FeO', None)
            CaO = request.POST.get('CaO', None)
            MgO = request.POST.get('MgO', None)
            Na2O = request.POST.get('Na2O', None)
            K2O = request.POST.get('K2O', None)
            H2O = request.POST.get('H2O', None)
            T = request.POST.get('T', None)
            P = request.POST.get('P', None)
            your_density = process_sample(SiO2, TiO2, Al2O3, Fe2O3, FeO, CaO, MgO, Na2O, K2O, H2O, P, T)
            return your_density
    else:
        form = PostForm()
...
def process_sample(iSiO2, iTiO2, iAl2O3, iFe2O3, iFeO, iCaO, iMgO, iNa2O, iK2O, iH2O, iP, iT):
    data = pandas.DataFrame({'SiO2': [iSiO2], 'TiO2': [iTiO2], 'Al2O3': [iAl2O3], 'Fe2O3':[iFe2O3], 'FeO': [iFeO], 'CaO':[iCaO], 'MgO': [iMgO], 'Na2O': [iNa2O], 'K2O': [iK2O], 'H2O': [iH2O], 'P': [iP], 'T':[iT]})
    ...
    columns = [data["Sample_ID"], data["Density_g-per-cm3"], data["Density_g-per-L"], norm_WP_H2O, data["T"], data["P"]]
...
def process_file(file_handle):
    data = pandas.read_excel('media/' + file_handle) #import excel file chosen by user
